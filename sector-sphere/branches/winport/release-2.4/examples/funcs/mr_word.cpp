...
#ifdef WIN32
    #ifdef MR_WORD_EXPORTS
        #define MR_WORD_API __declspec(dllexport)
    #else
        #define MR_WORD_API __declspec(dllimport)
    #endif
#else
    #define MR_WORD_API
#endif
...
int MR_WORD_API mr_word_map(const SInput* input, SOutput* output, SFile* file)
{
...
int MR_WORD_API mr_word_partition(const char* record, int size, void* param, int psize)
{
   if (NULL == record)
      return 0;
...
int MR_WORD_API mr_word_compare(const char* r1, int s1, const char* r2, int s2)
{
   char* p1 = (char*)r1;
   char* p2 = (char*)r2;
   while (*p1 != ' ')
...
int MR_WORD_API mr_word_reduce(const SInput* input, SOutput* output, SFile* file)
{
   vector<string> docs;
