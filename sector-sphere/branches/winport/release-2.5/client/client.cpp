...
int Client::init(const string& server, const int& port)
{
   ...
   m_KeepAlive = (HANDLE)_beginthreadex (NULL, 0, keepAlive, this, NULL, &ThreadID);
#endif
...
int Client::login(const string& username, const string& password, const char* cert)
{
   ...
   int iret = secconn.recv((char*)&m_iKey, 4);
   if (iret < 0 || m_iKey < 0)
