...
/**
 * Create a new directory in the Sector file system.
 */
public class Mkdir
{
    ...
    public static void main( String[] args )
    {
        ...
        if( args.length < 1 ) {
            System.out.println( "You must pass in the name of the directory " +
                                "to be created" );
            return;
        }
        ...
        try {
            ...
            status = client.sectorMkdir( path );
            if( status < 0 ) {
                System.out.println( "Failed to create " + path + ", status=" +
                                    status );
                return;
            }
        } catch( IOException e ) {
